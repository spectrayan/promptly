...
class PromptTest {
    ...
    private static Prompt draftWithTwoVersions() {
        // Build directly with two versions in the list.
        // We cannot use createNewVersion() twice because the domain rule for
        // DRAFT prompts is to update in-place rather than creating a new version.
        var versions = new ArrayList<PromptVersion>();
        versions.add(PromptVersion.builder()
                .versionNumber(1).content("Hello World")
                .changeMessage("Initial").createdBy("alice").build());
        versions.add(PromptVersion.builder()
                .versionNumber(2).content("Hello World v2")
                .changeMessage("Updated").createdBy("bob").build());
        return Prompt.builder()
                .id("p-1")
                .name("Test Prompt")
                .description("A test prompt")
    ...
    class VersioningTests {
        ...
        @Test
        @DisplayName("should update draft in place (no version bump)")
        void shouldUpdateDraftInPlace() {
            Prompt p = draftPrompt();

            p.createNewVersion("v1", "first", "alice");
            p.createNewVersion("v2", "second", "bob");
            PromptVersion v = p.createNewVersion("v3", "third", "carol");

            // DRAFT prompts update the latest version in place
            assertThat(v.getVersionNumber()).isEqualTo(1);
            assertThat(v.getContent()).isEqualTo("v3");
            assertThat(v.getCreatedBy()).isEqualTo("carol");
            assertThat(p.getCurrentVersion()).isEqualTo(1);
            assertThat(p.getVersions()).hasSize(1);
        }

        @Test
        @DisplayName("should still update in place after approval-reset cycle")
        void shouldUpdateInPlaceAfterApprovalReset() {
            Prompt p = draftPrompt();

            p.createNewVersion("v1", "first", "alice");
            assertThat(p.getCurrentVersion()).isEqualTo(1);

            // Approve and reset to draft — in-place update still applies
            // because status is back to DRAFT with existing versions
            p.markApproved();
            p.resetToDraft();
            PromptVersion v = p.createNewVersion("v2", "second", "bob");
