...
#
# Starts PostgreSQL, Backend (postgres profile), Frontend, seeds the database,
# runs Playwright E2E tests, and tears everything down.
#
# Usage:
#   ./apps/e2e/promptly-e2e/scripts/run-e2e.sh           # headless
#   ./apps/e2e/promptly-e2e/scripts/run-e2e.sh --headed  # headed mode
...
echo "═══════════════════════════════════════════════════════════"
echo "  Promptly — E2E Test Runner (PostgreSQL)"
echo "═══════════════════════════════════════════════════════════"
echo ""
...
while [ $attempts -lt 20 ]; do
    if docker exec promptly-postgres pg_isready -U promptly -d promptly >/dev/null 2>&1; then
...
if [ $attempts -ge 20 ]; then
    err "PostgreSQL did not become healthy."
    exit 1
fi

if ! lsof -i:8080 -sTCP:LISTEN -t >/dev/null ; then
    log "Starting Spring Boot backend..."
    PROMPTLY_PERSISTENCE_TYPE=postgres SPRING_PROFILES_ACTIVE=postgres mvn spring-boot:run -Ppersistence-postgres -f apps/backend/core/pom.xml > /tmp/backend.log 2>&1 &
    BACKEND_PID=$!
...
node scripts/generate-pg-seed.js
docker exec -i promptly-postgres psql -U promptly -d promptly < scripts/pg-seed.sql
ok "Database seeded."
